...
class ResNet(nn.Module):
    ...
    def forward(self, x, mode='output', target_class = [None], xMode=False):
        ...
        def generate_multi_relevance_cams(rel, layer_acts):
            ...
            rel_weights = self._compute_weights(rel, layer_acts, False)
            rel_cam = layer_acts * rel_weights
            rel_cam = torch.sum(rel_cam, dim=(1), keepdim=True)
        
            return xrel_cam, rel_cam
